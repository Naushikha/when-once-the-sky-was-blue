	...
	class FlyControls extends THREE.EventDispatcher {
		...
		constructor( object, domElement ) {
			...
			this.update = function ( delta ) {
				...
				// Start modifications
				scope.rotationVector.z = 0; 
				scope.object.rotation.z = 0;

				if (Math.abs(scope.object.rotation.x) > 0.3) {
					scope.object.rotation.x= 0.299 * scope.object.rotation.x/Math.abs(scope.object.rotation.x);
				}

				if (Math.abs(scope.object.rotation.y) > 0.3) {
					scope.object.rotation.y= 0.299 * scope.object.rotation.y/Math.abs(scope.object.rotation.y);
				}
				// End modifications
